...
function createMeter(file) {
  ...
  return { boot, record, aggregateAll, sessionUsage, sessionStatsOwn };
}
...
function apply(ctx, entryConfig) {
  ...
  const produceDeepSeek = async () => {
    const [snap, usage] = await Promise.all([collectOne("deepseek-official"), meter.aggregateAll()]);
    if (!snap) return fail("deepseek", "no-preset", "balance");
    if (snap.error) throw new Error(`DeepSeek: ${snap.error}`);
    return { provider: "deepseek", balance: snap.balance, usage };
  };
