...
const BUILTIN_PRICES = [
  /* deepseek-v4-flash official OFF-PEAK base (per DeepSeek pricing page):
     input(cache miss) $0.22/M, output $0.66/M, input(cache hit) $0.007/M.
     Peak hours are 2x off-peak: Mon-Fri 01-04 & 06-10 UTC; weekends are always off-peak.
     The per-row peak multiplier is applied at cost time (isDeepseekPeak). */
...
/**
 * DeepSeek peak-hour detection (UTC). Official: peak = Monday–Friday
 * 01:00–04:00 and 06:00–10:00 UTC; weekends and all other hours are off-peak.
 * Returns 2 (peak) or 1 (off-peak) — the price multiplier relative to the
 * off-peak base rates in BUILTIN_PRICES.
 */
function deepseekPeakMult(atMs) {
  const d = new Date(atMs);
  const day = d.getUTCDay(); // 0=Sun … 6=Sat
  if (day === 0 || day === 6) return 1; // weekend → always off-peak
  const h = d.getUTCHours();
  if ((h >= 1 && h < 4) || (h >= 6 && h < 10)) return 2; // peak
  return 1; // weekday off-peak
}
...
function apply(ctx, entryConfig) {
  ...
  /**
   * Session cost computed from the meter ledger rows (per-row timestamps), so
   * DeepSeek peak-hour rules apply correctly. For non-DeepSeek models (or when
   * the ledger has no rows for the session) falls back to aggregate estimate.
   */
  const sessionCost = async (sessionId, model, aggregateUsage, priceStateNow, fx) => {
    const isDeepSeek = /deepseek/i.test(String(model ?? ""));
    if (isDeepSeek) {
      const price = lookupPrice(model, priceStateNow.table);
      if (price === null) return null;
      let inputUsd = 0, cacheReadUsd = 0, outputUsd = 0;
      let rows = 0;
      for (const r of meter.rows()) {
        if (r.sessionId !== sessionId) continue;
        rows += 1;
        const mult = deepseekPeakMult(r.at);
        inputUsd += r.input * price.input * mult;
        cacheReadUsd += r.cacheRead * price.cacheRead * mult;
        outputUsd += r.output * price.output * mult;
      }
      if (rows === 0) return estimateCost(aggregateUsage, price, fx.rate);
      return {
        input: inputUsd * fx.rate,
        cacheRead: cacheReadUsd * fx.rate,
        output: outputUsd * fx.rate,
        total: (inputUsd + cacheReadUsd + outputUsd) * fx.rate
      };
    }
    const price = lookupPrice(model, priceStateNow.table);
    if (price === null) return null;
    return estimateCost(aggregateUsage, price, fx.rate);
  };
  ...
  /** GET /deepseek-quota — legacy shape: balance + daily/monthly/total cost. */
  const produceDeepSeek = async () => {
    ...
    // DeepSeek peak-hour rule: multiply by 2 during Mon–Fri peak, 1 otherwise
    // (weekends always off-peak). Applied only to deepseek models.
    ...
    for (const r of meter.rows()) {
      ...
      const mult = /deepseek/i.test(String(r.model ?? "")) ? deepseekPeakMult(r.at) : 1;
      const rowUsd = (r.input * price.input + r.cacheRead * price.cacheRead + r.output * price.output) * mult;
      totalCostUsd += rowUsd;
    ...
    if (sessionCache.key !== cacheKey || sessionCache.body === null || now - sessionCache.at >= cfg.sessionCacheTtlMs) {
      try {
        ...
        if (stats === null && usage === null) {
          ...
          const cost = usage === null ? null : await sessionCost(sessionId, model, usage, priceStateNow, fx);
          sessionCache = {
            key: cacheKey,
            at: now,
            body: JSON.stringify({
