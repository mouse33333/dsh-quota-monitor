	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			...
			tag.textContent = [
				...
				/* model section */
				".oq-mrow{display:flex;gap:8px;font-size:12.5px;align-items:baseline}",
				".oq-mrow-label{color:var(--dsw-alias-label-secondary);flex:none}",
				".oq-mrow-value{font-weight:600;flex:1;text-align:right;word-break:break-all}",
				...
				".oq-wrow-pct{font-weight:700;font-variant-numeric:tabular-nums;flex:none}",
		...
		/** One detail row: label + right-aligned value. */
		function mrow(label, value, key) {
			return (0, react_jsx_runtime.jsxs)("div", {
				className: "oq-mrow",
				children: [
					(0, react_jsx_runtime.jsx)("span", { className: "oq-mrow-label", children: label }, "label"),
					(0, react_jsx_runtime.jsx)("span", { className: "oq-mrow-value", children: value }, "value")
				]
			}, key);
		}

		/**
		 * Full task detail rows (turns/steps/durations, cache hit rate, tokens,
		 * estimated cost + price source) — the "everything else" the collapsed
		 * card defers to the expanded monitor.
		 */
		function TaskRows({ task, onRetry }) {
			...
			if (stats !== null && typeof stats === "object") {
				...
				if (stats.llmMs !== null && stats.llmMs !== void 0) parts.push(`LLM ${fmtDuration(stats.llmMs)}`);
				if (stats.toolMs !== null && stats.toolMs !== void 0) parts.push(`工具 ${fmtDuration(stats.toolMs)}`);
				rows.push(mrow("任务统计", parts.join(" · "), "stats"));
			}
			if (usage !== null && typeof usage === "object") {
				const totalIn = usage.uncachedInputTokens + usage.cacheReadTokens;
				rows.push(mrow("缓存命中率", `${cacheHitText(usage)}（缓存读取 ${fmtTokens(usage.cacheReadTokens)} / 输入 ${fmtTokens(totalIn)}）`, "hit"));
				rows.push(mrow("输入 tokens", `${fmtTokens(totalIn)}（未缓存 ${fmtTokens(usage.uncachedInputTokens)} · 缓存读取 ${fmtTokens(usage.cacheReadTokens)}）`, "input"));
				rows.push(mrow("输出 tokens", fmtTokens(usage.outputTokens), "output"));
			}
			if (cost !== null && cost !== void 0) {
				rows.push(mrow(
					"预估费用",
					`¥${fmtCost(cost.total)}（输入 ¥${fmtCost(cost.input)} · 输出 ¥${fmtCost(cost.output)} · 缓存 ¥${fmtCost(cost.cacheRead)}）`,
					"cost"
				));
			}
			if (prices !== null && prices !== void 0) {
				...
				rows.push(mrow("价格依据", `${source}${matched}${rate}（每日刷新）`, "prices"));
			}
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: rows });
		}
				...
				modelPart = (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, {
					children: [
						(0, react_jsx_runtime.jsxs)("div", {
							className: "oq-mrow",
							children: [
								(0, react_jsx_runtime.jsx)("span", { className: "oq-mrow-label", children: "模型" }, "label"),
								(0, react_jsx_runtime.jsx)("span", { className: "oq-mrow-value", children: current.model }, "value"),
						...
						(0, react_jsx_runtime.jsxs)("div", {
							className: "oq-mrow",
							children: [
								(0, react_jsx_runtime.jsx)("span", { className: "oq-mrow-label", children: "供应商" }, "label"),
								(0, react_jsx_runtime.jsx)("span", { className: "oq-mrow-value", children: current.provider }, "value")
							]
						}, "provider"),
						(current.reasoningEffort !== void 0 && current.reasoningEffort !== null
							? (0, react_jsx_runtime.jsxs)("div", {
								className: "oq-mrow",
								children: [
									(0, react_jsx_runtime.jsx)("span", { className: "oq-mrow-label", children: "推理强度" }, "label"),
									(0, react_jsx_runtime.jsx)("span", { className: "oq-mrow-value", children: String(current.reasoningEffort) }, "value")
								]
							}, "effort")
							: null),
						(0, react_jsx_runtime.jsxs)("div", {
							className: "oq-mrow",
							children: [
								(0, react_jsx_runtime.jsx)("span", { className: "oq-mrow-label", children: "路由状态" }, "label"),
								(0, react_jsx_runtime.jsx)("span", {
									className: "oq-mrow-value",
									style: { color: state.payload.routable === false ? "#ef5350" : "#2ecc71" },
									children: state.payload.routable === false ? "不可用" : "可用"
								}, "value")
							]
						}, "routable")
					]
		 ...
		 * @param props - slot props incl. the injected `sessionsApi` (Remote face
		 * of the sessions gateway) and `currentSessionId` (live reader).
		 */
		function QuotaMonitor({ sessionsApi, currentSessionId }) {
			...
			}, [sessionsApi, currentSessionId, fetchTask]);
			...
			if (!open) {
				...
				return (0, react_jsx_runtime.jsx)("button", {
					...
					children: [
						...
						/* zone 3: usage line (DeepSeek) or 5h progress bar (Go) */
						useDs
							? (0, react_jsx_runtime.jsx)("span", {
