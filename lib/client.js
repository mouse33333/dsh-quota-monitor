...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function DeepSeekBody({ state, task, onRetry }) {
			...
			const todayCost = state.payload.todayCost;
			const monthCost = state.payload.monthCost;
			const totalCost = state.payload.totalCost;
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, {
				children: [
					/* layer 1: 今日花费 — 最大最醒目 */
					(0, react_jsx_runtime.jsxs)("div", {
		...
		function QuotaMonitor({ sessionsApi, currentSessionId, subscribeModelStore }) {
			...
			if (!open) {
				...
				const dsTodayCost = deepseek.status === "ok" ? deepseek.payload.todayCost : null;
				const dsMonthCost = deepseek.status === "ok" ? deepseek.payload.monthCost : null;
