...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region styles
		const tagId = "dsh-quota-monitor/Monitor.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			...
			tag.textContent = [
				...
				/* zone 5: footer */
				".oq-card .oq-c-time{grid-column:1/-1;display:flex;align-items:center;justify-content:flex-end;gap:6px;color:var(--dsw-alias-label-tertiary,var(--dsw-alias-label-secondary));font-size:10px;max-width:none;letter-spacing:.02em}",
				".oq-card .oq-c-refresh{cursor:pointer;font-size:13px;line-height:1;opacity:.55;transition:opacity .15s;flex:none}",
				".oq-card .oq-c-refresh:hover{opacity:1}",
				/* click-away backdrop */
				".oq-backdrop{position:absolute;inset:0;z-index:25;pointer-events:auto;background:transparent}",
				/* monitor window */
				".oq-panel{position:absolute;width:384px;max-width:calc(100vw - 32px);max-height:calc(100vh - 32px);overflow:hidden;box-sizing:border-box;display:flex;flex-direction:column;gap:10px;padding:12px;border-radius:14px;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);font-size:13px;line-height:1.45;box-shadow:0 12px 40px rgba(0,0,0,.3);z-index:30;pointer-events:auto}",
				/* detail region: tabs + a grid that stacks every tab in one cell,
				   so the panel height always equals the tallest tab (no jump) */
				".oq-detail{flex:1;min-height:0;display:flex;flex-direction:column;gap:10px}",
				".oq-detail-body{display:grid;grid-template-columns:1fr;min-height:0;overflow-y:auto}",
				".oq-detail-body > .oq-sec{grid-area:1 / 1}",
				...
				/* deepseek: 4-layer visual hierarchy */
				".oq-ds-layer1{display:flex;align-items:baseline;gap:10px;padding:6px 0 2px}",
				".oq-ds-layer1-value{font-size:26px;font-weight:800;font-variant-numeric:tabular-nums;line-height:1.15}",
				".oq-ds-layer2{display:flex;align-items:baseline;gap:8px;padding:6px 0;border-top:1px solid var(--dsw-alias-border-l1)}",
				".oq-ds-layer2-value{font-size:16px;font-weight:700;font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-primary)}",
				".oq-ds-layer3{display:flex;align-items:baseline;gap:8px;padding:6px 0;border-top:1px solid var(--dsw-alias-border-l1)}",
				".oq-ds-layer3-value{font-size:13px;font-weight:600;font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-secondary)}",
				".oq-ds-layer4{display:flex;align-items:center;gap:8px;padding:6px 0;border-top:1px solid var(--dsw-alias-border-l1)}",
				".oq-ds-layer4-value{font-size:12px;font-weight:600;font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-tertiary,var(--dsw-alias-label-secondary))}",
				".oq-ds-layer-label{font-size:11px;color:var(--dsw-alias-label-tertiary,var(--dsw-alias-label-secondary));min-width:42px;flex:none}",
				".oq-ds-sub{width:100%;color:var(--dsw-alias-label-tertiary,var(--dsw-alias-label-secondary));font-size:10px;font-variant-numeric:tabular-nums;padding-top:2px}",
				...
				/* tab bar: fixed height so it never shifts when labels change */
				".oq-tabs{display:flex;gap:6px;flex-wrap:nowrap;align-items:center;height:30px}",
				".oq-tab{appearance:none;font:inherit;cursor:pointer;border:1px solid var(--dsw-alias-border-l1);background:transparent;color:var(--dsw-alias-label-secondary);border-radius:8px;height:26px;line-height:24px;padding:0 12px;font-size:12px;white-space:nowrap}",
				".oq-tab:hover{border-color:var(--dsw-alias-border-l2);color:var(--dsw-alias-label-primary)}",
				".oq-tab[data-active=true]{border-color:var(--dsw-alias-accent-fg,#588eff);color:var(--dsw-alias-accent-fg,#588eff);background:rgba(88,142,255,.12);font-weight:600}",
				/* qwen tab: static info + external link */
				".oq-qwen-info{display:flex;flex-direction:column;gap:10px}",
				".oq-qwen-desc{margin:0;color:var(--dsw-alias-label-secondary);font-size:12px;line-height:1.6}",
				".oq-qwen-link{display:inline-flex;align-items:center;gap:6px;align-self:flex-start;padding:6px 14px;border-radius:8px;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);color:var(--dsw-alias-accent-fg,#588eff);font-size:13px;font-weight:600;text-decoration:none;transition:border-color .15s,background .15s}",
				".oq-qwen-link:hover{border-color:var(--dsw-alias-accent-fg,#588eff);background:rgba(88,142,255,.1)}"
			].join("");
		...
		const POS_KEY = "dsh-quota-monitor.pos";
		...
		function planTag(provider) {
			...
			if (p.includes("minimax")) return "MiniMax Token Plan";
			return provider ?? "";
		}
		...
		/** Whether a provider id belongs to the MiniMax Token Plan. */
		function isMinimaxPlan(provider) {
			return /minimax/i.test(String(provider ?? ""));
		}
		//#endregion
		...
		/** MiniMax Token Plan section body for a state (windows-shaped payload). */
		function MiniMaxBody({ state, onRetry }) {
			if (state.status === "loading") {
				return (0, react_jsx_runtime.jsx)("div", { className: "oq-loading", children: "加载中…" });
			}
			if (state.status === "error") {
			...
			const windows = state.payload && Array.isArray(state.payload.windows) ? state.payload.windows : [];
			if (windows.length === 0) {
				return (0, react_jsx_runtime.jsx)("div", { className: "oq-loading", children: "暂无 MiniMax 额度数据" }, "none");
			}
			return windows.map((win) =>
				(0, react_jsx_runtime.jsx)(WindowRow, { label: win.label || windowLabel(win.key), win }, win.key)
			);
		}
		...
		function DeepSeekBody({ state, task, onRetry }) {
			if (state.status === "loading") {
				return (0, react_jsx_runtime.jsx)("div", { className: "oq-loading", children: "加载中…" });
			}
			if (state.status === "error") {
			...
			const todayCost = state.payload.todayCost ?? null;
			const monthCost = state.payload.monthCost ?? null;
			const totalCost = state.payload.totalCost ?? null;
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, {
				children: [
					/* layer 1: 今日花费 — 最大最醒目 */
					(0, react_jsx_runtime.jsxs)("div", {
						className: "oq-ds-layer1",
						children: [
							(0, react_jsx_runtime.jsx)("span", { className: "oq-ds-layer-label", children: "今日" }, "label"),
							(0, react_jsx_runtime.jsx)("span", {
								className: "oq-ds-layer1-value oq-money",
								children: todayCost !== null ? `¥${fmtCost(todayCost.total)}` : "—"
							}, "value")
						]
					}, "layer1"),
					/* layer 2: 当月花费 — 中等 */
					(0, react_jsx_runtime.jsxs)("div", {
						className: "oq-ds-layer2",
						children: [
							(0, react_jsx_runtime.jsx)("span", { className: "oq-ds-layer-label", children: "当月" }, "label"),
							(0, react_jsx_runtime.jsx)("span", {
								className: "oq-ds-layer2-value",
								children: monthCost !== null ? `¥${fmtCost(monthCost.total)}` : "—"
							}, "value")
						]
					}, "layer2"),
					/* layer 3: 总花费 — 次要 */
					(0, react_jsx_runtime.jsxs)("div", {
						className: "oq-ds-layer3",
						children: [
							(0, react_jsx_runtime.jsx)("span", { className: "oq-ds-layer-label", children: "总花费" }, "label"),
							(0, react_jsx_runtime.jsx)("span", {
								className: "oq-ds-layer3-value",
								children: totalCost !== null ? `¥${fmtCost(totalCost.total)}` : "—"
							}, "value")
						]
					}, "layer3"),
					/* layer 4: 余额 — 最小 */
					(0, react_jsx_runtime.jsxs)("div", {
						className: "oq-ds-layer4",
						children: [
							(0, react_jsx_runtime.jsx)("span", { className: "oq-ds-layer-label", children: "余额" }, "label"),
							(0, react_jsx_runtime.jsx)("span", { className: "oq-ds-layer4-value", children: fmtMoney(balance.totalBalance) }, "value"),
							(0, react_jsx_runtime.jsx)("span", {
		...
		function QuotaMonitor({ sessionsApi, currentSessionId, subscribeModelStore }) {
			...
			const [minimax, setMinimax] = react.useState({ status: "loading", at: new Date() });
			...
			const refresh = react.useCallback(() => {
				...
				/* MiniMax Token Plan lives in the /quota-providers aggregate (windows-shaped). */
				fetchRoute("/quota-providers").then((res) => {
					if (res.status === "ok" && res.payload && Array.isArray(res.payload.providers)) {
						const entry = res.payload.providers.find((p) => p.id === "minimax-cn");
						if (entry && entry.windows) {
							setMinimax({ status: "ok", payload: { windows: entry.windows }, at });
						} else if (entry && entry.error) {
							setMinimax({ status: "error", error: entry.error, at });
						} else {
							setMinimax({ status: "error", error: "未配置 minimax-cn 供应商", at });
						}
					} else {
						setMinimax({ status: "error", error: res.error || "unexpected response", at });
					}
				}).catch((error) => {
					setMinimax({ status: "error", error: String((error && error.message) || error), at });
				});
				const sessionId = typeof currentSessionId === "function" ? currentSessionId() : void 0;
			...
			const latest = [opencode.at, deepseek.at, model.at, minimax.at].reduce((a, b) => (a > b ? a : b));
			...
			if (!open) {
				...
				const useMini = provider !== null && isMinimaxPlan(provider);
				...
				const dsTodayCost = deepseek.status === "ok" ? (deepseek.payload.todayCost ?? null) : null;
				const dsMonthCost = deepseek.status === "ok" ? (deepseek.payload.monthCost ?? null) : null;
				/* MiniMax windows (from /quota-providers): rolling + weekly only. */
				const mmWindows = minimax.status === "ok" && minimax.payload && Array.isArray(minimax.payload.windows) ? minimax.payload.windows : [];
				const mmRolling = mmWindows.find((w) => w.model_name === "general" && w.key === "rolling") || mmWindows.find((w) => w.key === "rolling") || null;
				const mmWeekly = mmWindows.find((w) => w.key === "weekly") || null;
				...
				/* Unified windows view for Go OR MiniMax (both are windows-shaped). */
				const winHeadline = useMini ? (mmRolling ?? mmWeekly) : goHeadline;
				const winRolling = useMini ? mmRolling : goRolling;
				const rollingPct = (useMini ? mmRolling : goRolling) === null ? null : Math.round((useMini ? mmRolling : goRolling).percent);
				const weeklyPct = (useMini ? mmWeekly : goWeekly) === null ? null : Math.round((useMini ? mmWeekly : goWeekly).percent);
				...
				const cardTitle = [
					...
					useDs && dsBalance
						...
						: useMini
							? (minimax.status === "ok"
								? `MiniMax Token Plan：5h 已用 ${rollingPct === null ? "…" : `${rollingPct}%`}（余 ${mmRolling === null ? "…" : Math.round(mmRolling.percentRemaining)}%）· 周 ${weeklyPct === null ? "…" : `${weeklyPct}%`}`
								: "MiniMax 套餐数据不可用")
							: opencode.status === "ok"
				...
				const heroSub = useDs
					...
					: (winHeadline === null
						? "…"
						: `余 ${Math.round(winHeadline.percentRemaining)}% · 周${weeklyPct === null ? "…" : `${weeklyPct}%`}${useMini ? "" : ` · 月${monthlyPct === null ? "…" : `${monthlyPct}%`}`}`);
				return (0, react_jsx_runtime.jsx)("button", {
					...
					children: [
						...
						useDs
							...
							: (0, react_jsx_runtime.jsxs)("span", {
								className: "oq-c-hero",
								style: { color: winHeadline === null ? "#ef5350" : percentColor(winHeadline.percent) },
								children: [
									(0, react_jsx_runtime.jsx)("span", { className: "oq-c-hero-prefix", children: "5h " }, "pre"),
									(0, react_jsx_runtime.jsx)("span", { children: winHeadline === null ? "…" : `${Math.round(winHeadline.percent)}%` }, "amt")
								]
							}, "plan"),
						...
						useDs
							? (0, react_jsx_runtime.jsx)("span", {
								...
								children: dsTodayCost === null && dsMonthCost === null
									? "暂无花费"
									: `今日 ¥${fmtCost(dsTodayCost?.total ?? 0)} · 本月 ¥${fmtCost(dsMonthCost?.total ?? 0)}`
							}, "usage")
								...
								children: [
									(0, react_jsx_runtime.jsx)("span", {
										...
										children: (0, react_jsx_runtime.jsx)("i", {
											style: {
												width: `${winHeadline === null ? 0 : clamp(winHeadline.percent, 0, 100)}%`,
												background: winHeadline === null ? "#ef5350" : percentColor(winHeadline.percent)
											}
										})
									}, "bar"),
									(0, react_jsx_runtime.jsx)("span", {
										className: "oq-c-usage",
										children: `5h 重置倒计时 · ${winRolling === null ? "…" : formatCountdown(winRolling.resetsAt)}`
									}, "countdown")
								]
							}, "usage"),
						...
						/* zone 5: footer with refresh button */
						(0, react_jsx_runtime.jsxs)("div", {
							className: "oq-c-time",
							children: [
								(0, react_jsx_runtime.jsx)("span", {
									className: "oq-c-refresh",
									title: "刷新",
									"aria-label": "刷新额度数据",
									onClick: (e) => { e.stopPropagation(); refresh(); },
									children: "⟳"
								}, "refresh"),
								(0, react_jsx_runtime.jsx)("span", { children: `更新 ${formatTime(latest)}` }, "text")
							]
						}, "time")
					]
				});
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsxs)("div", { className: "oq-detail", children: [
								/* tab bar */
								(0, react_jsx_runtime.jsx)("div", {
									className: "oq-tabs",
									children: [
										...
										(0, react_jsx_runtime.jsx)("button", {
											type: "button",
											className: "oq-tab",
											"data-active": tab === t.id ? "true" : void 0,
											onClick: () => setTab(t.id),
											children: t.label
										}, t.id)
									)
								}, "tabs"),
								/* all tabs stacked in one grid cell: panel height = tallest tab */
								(0, react_jsx_runtime.jsxs)("div", { className: "oq-detail-body", children: [
									(0, react_jsx_runtime.jsxs)("section", {
										className: "oq-sec",
										style: { visibility: tab === "model" ? "visible" : "hidden" },
										children: [
											(0, react_jsx_runtime.jsx)("h3", { className: "oq-sec-title", children: "任务详情" }, "title"),
											(0, react_jsx_runtime.jsx)(ModelBody, { state: model, task, onRetry: refresh }, "body")
										]
									}, "sec-model"),
									(0, react_jsx_runtime.jsxs)("section", {
										className: "oq-sec",
										style: { visibility: tab === "opencode" ? "visible" : "hidden" },
										children: [
									...
									(0, react_jsx_runtime.jsxs)("section", {
										className: "oq-sec",
										style: { visibility: tab === "deepseek" ? "visible" : "hidden" },
										children: [
											(0, react_jsx_runtime.jsx)("h3", { className: "oq-sec-title", children: "DeepSeek API" }, "title"),
											(0, react_jsx_runtime.jsx)(DeepSeekBody, { state: deepseek, task, onRetry: refresh }, "body")
										]
									}, "sec-deepseek"),
									(0, react_jsx_runtime.jsxs)("section", {
										className: "oq-sec",
										style: { visibility: tab === "minimax" ? "visible" : "hidden" },
										children: [
											(0, react_jsx_runtime.jsx)("h3", { className: "oq-sec-title", children: "MiniMax Token Plan" }, "title"),
											(0, react_jsx_runtime.jsx)(MiniMaxBody, { state: minimax, onRetry: refresh }, "body")
										]
									}, "sec-minimax"),
									(0, react_jsx_runtime.jsxs)("section", {
										className: "oq-sec",
										style: { visibility: tab === "qwen" ? "visible" : "hidden" },
										children: [
											(0, react_jsx_runtime.jsx)("h3", { className: "oq-sec-title", children: "千问 Token Plan" }, "title"),
											(0, react_jsx_runtime.jsxs)("div", {
												className: "oq-qwen-info",
												children: [
													(0, react_jsx_runtime.jsx)("p", {
														className: "oq-qwen-desc",
														children: "千问 Token Plan 不提供额度查询 API，需前往控制台查看。"
													}, "desc"),
													(0, react_jsx_runtime.jsx)("a", {
														className: "oq-qwen-link",
														href: "https://platform.qianwenai.com/home/analytics/token-plan/individual",
														target: "_blank",
		...
		//#region SettingsPanel (settings.section entry -> visible in plugin list / settings)
		/**
		 * A small status panel shown in the Settings > 额度监控 section. It polls
		 * the host's `/quota-providers` snapshot (which already includes every
		 * configured provider, e.g. opencode-go, deepseek-official, minimax-cn).
		 */
		function SettingsPanel() {
			const [state, setState] = react.useState({ status: "loading", at: new Date() });
			const refresh = react.useCallback(() => {
				const at = new Date();
				fetchRoute("/quota-providers").then((res) => {
					if (res.status === "ok" && res.payload && Array.isArray(res.payload.providers)) {
						setState({ status: "ok", providers: res.payload.providers, at });
					} else {
						setState({ status: "error", error: res.error || "unexpected response", at });
					}
				});
			}, []);
			react.useEffect(() => {
			...
			if (state.status === "error") {
				return (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						(0, react_jsx_runtime.jsx)("span", { children: `获取失败：${state.error}` }, "msg"),
						(0, react_jsx_runtime.jsx)("button", { type: "button", className: "oq-err-btn", onClick: refresh, children: "重试" }, "retry")
					]
				});
			}
			return (0, react_jsx_runtime.jsxs)("div", {
				className: "oq-sec",
				children: [
					(0, react_jsx_runtime.jsx)("h3", { className: "oq-sec-title", children: "已监控供应商" }, "title"),
					...state.providers.map((p) => {
						const ok = !p.error;
						return (0, react_jsx_runtime.jsxs)("div", {
							className: "oq-urow",
							children: [
								(0, react_jsx_runtime.jsx)("span", { className: "oq-urow-label", children: p.label || p.id }, "l"),
								(0, react_jsx_runtime.jsx)("span", {
									className: ok ? "oq-chip oq-chip-ok" : "oq-chip oq-chip-bad",
									children: ok ? "正常" : "异常"
								}, "v")
							]
						}, p.id);
					}),
					(0, react_jsx_runtime.jsx)("div", { className: "oq-foot", children: `更新于 ${formatTime(state.at)} · 每 ${Math.round(POLL_INTERVAL_MS / 60000)} 分钟刷新` }, "foot")
				]
			});
		}
		//#endregion
		...
		function apply(ctx) {
			...
			// Register a Settings section so the plugin shows up in the web
			// plugin/settings list with a clickable status panel. Per the
			// dsh-client-ui-settings-plugins contract, the component is the
			// SECOND argument to ctx.slots.register (not returned by inject).
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				name: "settings.section",
				id: "dsh-quota-monitor-settings",
				label: "额度监控",
				order: 60
			}, SettingsPanel));
		}
