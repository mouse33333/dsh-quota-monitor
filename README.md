...
- **收起态**：一张**可拖拽**的状态条式摘要卡片，**宽度跟随侧边栏**（约 `侧边栏宽 - 16px`），
  聚焦**当前使用的模型**，**与聊天框模型选择实时同步**（切换 DeepSeek ↔ OpenCode Go
  的瞬间卡片即切换对应套餐视图）：
  ...
  - 第 2-3 行：**当前套餐的额度/余额** —— DeepSeek 模型显示 `¥余额`、可用状态、
    ...
    **5小时 → 周 → 月** 优先级展示：`5h 已用%`（大字，按使用率着色）+ 5h 进度条 +
    `余% · 周% · 月%` 并列一行 + **5h 重置倒计时**（`5h 重置倒计时 · 1 小时 7 分后`）；
...
- **展开态**：384px 宽的监控面板（自动避让侧边栏，锚定在内容区左下角；
  **点击空白处或按 Esc 自动关闭**，也可点 ×），展示**所有绑定套餐**与全部任务详情：
  - **当前模型**：模型名大标题 + 套餐标签 + 单行元信息（供应商 · 推理强度 · 路由状态）；
  - **任务指标网格**：4 列紧凑网格 —— 轮数/步数/LLM/工具时长、缓存命中率（按健康度着色）、
    输入/输出 tokens、**预估费用**（金色高亮）；缓存拆分与费用分项在单元格 tooltip 中；
    价格依据单行脚注（价格源、匹配模型、汇率、每日刷新）；
  - **OpenCode Go 套餐**：5小时/本周/本月三行 —— 进度条 + 已用%（大字）+ 余量% + 重置时间与倒计时；
